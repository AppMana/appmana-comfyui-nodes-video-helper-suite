...
def send_sync(*args, **kwargs):
    ...
    ctx.server.send_sync(*args, **kwargs)


def client_id() -> str | None:
    ctx = current_execution_context()
    return ctx.server.client_id
...
class WrappedPreviewer(latent_preview.LatentPreviewer):
    ...
    def decode_latent_to_preview_image(self, preview_format, x0):
        ...
        if self.first_preview:
            ...
            send_sync('VHS_latentpreview', {'length': num_images, 'rate': self.rate})
    ...
    def process_previews(self, image_tensor, ind, leng):
        ...
        for preview in previews_ubyte:
            ...
            send_sync(BinaryEventTypes.PREVIEW_IMAGE,
                      message.getvalue(), client_id())
            ind = (ind + 1) % leng
...
def get_latent_video_previewer(device, latent_format, *args, **kwargs):
    ...
    try:
        serv = ctx.server
        assert hasattr(serv, "prompt_queue"), "running distributed, which isn't going to support this logic"
